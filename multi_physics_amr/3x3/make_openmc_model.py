...
uo2.set_density('atom/cm3', 6.9334999+22)
...
fuel_cell  = openmc.Cell(cell_id=13, name="UO2 Fuel Pin", fill=uo2,
                          region=-fuel_or)
gap_cell   = openmc.Cell(cell_id=14, name="Pin Gap", fill=helium,region=+fuel_or & -clad_ir)       # void gap
...
lattice.lower_left = (-1.5 * pin_pitch, -1.5 * pin_pitch)
lattice.universes = [[pin_universe] * 3 for _ in range(3)]


TOP_PAD = 0.0   # cm of extra reflector above active_core_height, if any

assembly_half = 1.5 * pin_pitch
...
root_cell = openmc.Cell(cell_id=27, name="3x3 Fuel Lattice Cell", fill=lattice,

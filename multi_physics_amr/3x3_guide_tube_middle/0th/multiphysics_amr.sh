...
#SBATCH --job-name=gt_mid_base_case_uniform
#SBATCH --error=gt_mid_base_case_uniform.%J.err
#SBATCH --output=gt_mid_base_case_uniform.%J.out
...
srun apptainer exec \
  ...
  cd ${input_path} && ${CARDINAL} -i openmc_mesh.i --mesh-only --n-threads=1 && \
  ${CARDINAL} -i openmc.i --n-threads=1"